...
def compute_class_weights(
    ...
    if method.lower() == "balanced":
        ...
        if np.any(present_mask):
            ...
            weights[present_mask] = (1.0 - beta) / np.maximum(
                effective_num, np.finfo(np.float64).eps
            )
...
if __name__ == "__main__":
    ...
    from nebula.data.dataset import GalaxyDatasetSource

    dataset = GalaxyDatasetSource(
        csv_or_json_file=Path("data/source/source_galaxy_labels.json"),
        img_dir=Path("data/source/galaxy_images_rgb"),
        transform=transforms.ToTensor(),
        include_rotations=False,
    )

    weights = compute_class_weights(dataset.df["label"])
